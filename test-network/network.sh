...
function networkUp() {
  ...
  if [ "${DATABASE}" == "elasticsearch" ]; then
    COMPOSE_FILES="${COMPOSE_FILES} -f ${COMPOSE_FILE_ES}"
  fi
...
function networkDown() {
  ...
  docker-compose -f $COMPOSE_FILE_BASE -f $COMPOSE_FILE_COUCH -f $COMPOSE_FILE_ES -f $COMPOSE_FILE_CA down --volumes --remove-orphans
...
COMPOSE_FILE_ES=docker/docker-compose-es.yaml
